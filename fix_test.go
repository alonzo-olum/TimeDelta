...
// TestApplyStartEnd validates that Fix only returns points that lie within the start/end range provided.
func TestApplyStartEnd(t *testing.T) {
	in := []Point{
		{0.5, 100},
		{8.4, 110},
		{32.5, 120},
...
// TestApplyStartEndNaNs validates that Fix adds NaN's at the front or end as needed to match the requested range.
func TestApplyStartEndNaNs(t *testing.T) {
	in := []Point{
		{0.5, 100},
		{41.3, 110},
	}
...
// TestApplyIrregularStartEndNaNs validates that Fix honors start/end even when they are irregular (when they are not divisible by the interval).
func TestApplyIrregularStartEndNaNs(t *testing.T) {
	in := []Point{
		{0.5, 100},
		{41.3, 110},
	}
...
// TestIrregularStartEndIrregularTimestamps validates that Fix correctly handles the combination
...
func TestIrregularStartEndIrregularTimestamps(t *testing.T) {
	in := []Point{
		{0.5, 105},
		{1, 120},
		{1.5, 130},
